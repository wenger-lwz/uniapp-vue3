<template>
  <section class="md:max-w-1200px m-a w-100%" v-if="Object.keys(orderDetail).length">
    ...
    <div class="p-x-20px md:p-x-0" v-if="Object.keys(orderDetail).length">
      ...
      <div class="flex items-center mb20px" v-for="item in orderDetail.product">
      ...
      <n-card class="bg-stone-50 mb-20px mt-38px text-14px px-16px" style="box-shadow: none; border: none">
        <div class="flex justify-between items-center" v-if="orderDetail.shipp_price">
          <span>{{ $t('myOrder.freight') }}</span>
          <span>
            {{ orderDetail.currencys.symbol_left + orderDetail.shipp_price + orderDetail.currencys.symbol_right }}
          </span>
        </div>
        <div class="flex justify-between items-center mt20px mb20px" v-if="orderDetail.insurance">
        ...
        <div class="flex justify-between items-center mt20px mb20px" v-if="orderDetail.tips">
          <span>{{ $t('myOrder.tip') }}</span>
          <span>{{ orderDetail.currencys.symbol_left + orderDetail.tips + orderDetail.currencys.symbol_right }}</span>
        </div>
        <div class="flex justify-between items-center" v-if="orderDetail.tax">
      ...
      <div class="flex-center justify-around h-100%" v-if="orderDetail.status == 0">
        <n-button size="large" tag="a" type="success" class="p-x24px p-y9px">
          <nuxt-link :href="'/checkouts/' + orderDetail.okey">
            {{ $t('goodsOrder.toPay') }}
          </nuxt-link>
        </n-button>
      </div>
...
const {
  ...
  error,
  refresh,
} = await useAsyncData(() => getOrderDetailAPI(useRoute().fullPath), {
  transform(data) {
