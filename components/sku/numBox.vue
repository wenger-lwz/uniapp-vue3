<template>
  ...
  <n-space vertical class="mb-20px">
    <n-space justify="start" align="center">
      <n-button size="large" :disabled="inputValue == minValue" @click="change('-')" text class="text-30px font-500">
        -
      </n-button>
      <p id="product_num" class="text-15px color-#000 ml-15px mr-15px">{{ inputValue }}</p>
      <n-button size="large" @click="change('+')" text class="text-30px font-500">+</n-button>
    </n-space>
  </n-space>
  <!-- <div
    class=" numBox"
 ...
 * @property {Nunber} width 宽度(默认值: 90)。
 * @property {Nunber} height 高度(默认值: 32)。
...
export default {
  ...
  methods: {
    ...
    change(type) {
      ...
      if (type == '+') {
        if (Number(Number(this.maxValue)) == -1 || this.inputValue < Number(Number(this.maxValue))) {
          value += this.step
    ...
    correctingNum(value) {
      ...
      if (value !== '') {
        ...
        } else if (Number(this.maxValue) !== -1 && value > Number(this.maxValue)) {
          // 如果用户输入的内容大于最大值 则等于 最大值
          this.$nextTick(() => {
            this.inputValue = Number(this.maxValue).toFixed(decimal)
          })
