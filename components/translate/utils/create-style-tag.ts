 ...
 * @param options
 */
export function createStyleTag(css: string, options: CreateStyleTagOptions = {}): CreateStyleTagReturn {
  let isLoaded = false

  const { immediate = true, id = `google-translate-select_${++_id}`, document = defaultDocument } = options
