import { useBoolean, useLoading } from '@/utils/hooks'
import type { AxiosInstance, AxiosRequestConfig, CancelTokenSource } from 'axios'
import type { Ref } from 'vue'
import axios from 'axios'
import { ref } from 'vue'
import { cancelRequestList } from './cancelrequestApi'
...
export function createRequest(axiosConfig: AxiosRequestConfig, backendConfig?: Service.BackendResultConfig) {
  ...
  async function asyncRequest<T>(param: RequestParam): Promise<Service.RequestResult<T>> {
    ...
    if (cancelRequestList.includes(url) && cancelTokenSource) {
...
export function createHookRequest(axiosConfig: AxiosRequestConfig, backendConfig?: Service.BackendResultConfig) {
  ...
  function post<T>(url: string, data?: any, config?: AxiosRequestConfig) {
    ...
    return useRequest<T>({ url, method: 'post', data: fd, axiosConfig: config })
  }
  /**
   * put请求
   * @param url - 请求地址
