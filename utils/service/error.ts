import { DEFAULT_REQUEST_ERROR_CODE, DEFAULT_REQUEST_ERROR_MSG, ERROR_STATUS, NETWORK_ERROR_CODE, NETWORK_ERROR_MSG, REQUEST_TIMEOUT_CODE, REQUEST_TIMEOUT_MSG } from '@/utils/config'
...
export function handleAxiosError(axiosError: AxiosError) {
  ...
  const actions: Common.StrategyAction[] = [
    [
      // 网路错误
      (process.client && !window?.navigator?.onLine) || axiosError.message === 'Network Error',
      () => {
    ...
    [
      // 超时错误
      process.client && axiosError.code === REQUEST_TIMEOUT_CODE && axiosError.message.includes('timeout'),
      () => {
