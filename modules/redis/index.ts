...
export type ModuleOptions = {
  ...
  key?: (req: IncomingMessage, crypto: any) => string | false
}
...
export default defineNuxtModule<ModuleOptions>({
  ...
  setup(_options, nuxt) {
    ...
    nuxt.hook('nitro:config', (nitro) => {
      ...
      nitro.virtual['#elastic-cache-options'] = `export const options = ${JSON.stringify(
        options,
        function (key, val) {
...
function sanitizeOptions(options: ModuleOptions) {
  return {
    ...options,
    pages: Array.isArray(options.pages) ? options.pages.filter(Boolean) : [options.pages].filter(Boolean),
  }
}
